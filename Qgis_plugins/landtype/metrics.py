...
class ConfMatrix():
    ...
    def __init__(self, num_classes):
        ...
        self.classes_percentage = np.zeros((self.num_classes))
    ...
    def add(self, gt, pred):
        ...
        if not gt.size == 0:
            ...
            for i in range(self.num_classes):
                self.classes_percentage[i] += np.sum(gt == i)
    ...
    def get_aa_micro(self):
        confmatrix = self.norm_on_lines()
        return np.sum(np.diagonal(confmatrix) * (self.classes_percentage / np.sum(self.classes_percentage)))
    ...
    def get_mIoU_micro(self):
        return np.sum(self.get_IoU() * (self.classes_percentage / np.sum(self.classes_percentage)))

    def get_mIoU_per_class(self):
        return self.get_IoU()
